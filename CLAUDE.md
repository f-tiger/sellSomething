...
5. **自动进化(owner 2026-08-22「新加进来的2个站点也要具备自动扩展，自动进化
   能力」)**:舰队每日进化 Routine(05:40 UTC)覆盖本仓——每轮至多一项、
   **只做加法**、严格遵循本文件既有约定、动手前必先 `git pull --rebase`
   (本仓另有活跃会话,冲突时以其为先)。信号源:KV `SUBSCRIBERS` 增量、
   七站 worker 的真实使用量、本仓 trend digest;无信号轮次只做健康检查
   (CI 绿否、七站域名在线否),不硬凑改动。既有周循环(Superteam 赏金草稿、
   x402 状态)保持原样,不重复不替代。
